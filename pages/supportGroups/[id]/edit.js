...
const SupportGroups = ({ hcworkers, data }) => {
  ...
  const [form, setForm] = useState({
    id: data?.id,
    supportMeetingDate: new Date(data?.supportmeetingdate)
      .toISOString()
      .slice(0, 10),
  ...
  const submitForm = () => {
    ...
    axios
    axios
      .put(`${process.env.NEXT_PUBLIC_SERVER_URL}/support_groups/update`, form)
      .then((data) => {
        console.log(data);
        notifyMessage('ok');
        if (data.status === 200) {
  ...
  const displayUserList = () => {
    return (
      hcworkers &&
      /* hcworkers.filter(user=>user.userrole !=='DES') */ hcworkers.map(
        (user, index) => {
          return (
      ...
      <Layout>
        <SubHeader pageTitle={"Support Group Record"}>
          {
            <Link href="/supportGroups/pastEvents">
              <button className=" rounded bg-middle-purple text-center px-5 py-1 shadow-xl rounded-lg flex items-center block">
                <img
                  src="/search_icon.svg"
                  alt="review past group event icon"
                  width={27}
                />
        ...
        <div className="my-10">
          <section className="container mx-auto  bg-white rounded-xl shadow-md">
            <div id="form" className="grid grid-cols-1 gap-10  p-7 mb-5">
              <div className="flex items-center gap-x-3">
                <img
                  src="/support_groups/Support_Group_information.png"
                  alt=""
                />
              ...
              <label className="text-xl font-medium flex flex-col items-start  gap-5">
                ...
                <input
                  type="date"
                  defaultValue={form.supportMeetingDate}
              ...
              <label className="text-xl font-medium flex flex-col items-start  gap-5">
                ...
                <input
                  type="time"
                  defaultValue={form.supportGroupStartTime}
                  name="supportGroupStartTime"
                  onChange={handleForm}
                  className="border-black rounded p-2 mb-2 block "
                />
              </label>
              <label className="text-xl font-medium flex flex-col items-start  gap-5">
                ...
                <input
                  type="time"
                  defaultValue={form.supportGroupEndTime}
                  name="supportGroupEndTime"
                  onChange={handleForm}
                  className="border-black rounded p-2 mb-2 block"
                />
              </label>
              <label className="text-xl font-medium flex flex-col gap-5">
                ...
                <input
                  type="text"
                  defaultValue={form.supportGroupName}
              ...
              <label className="text-xl font-medium flex flex-col gap-5">
                ...
                >
                  <option>{form?.supportGroupAudience}</option>
                  {/* <option onClick={() => setSearchByUser("All")}>All</option> */}
                  {form.supportGroupAudience === "mens" ? null : (
                    <option value="mens">Mens</option>
                  )}
                  {form.supportGroupAudience === "womens" ? null : (
                    <option value="womens">Womens</option>
                  )}
                  {form.supportGroupAudience === "men and women" ? null : (
              ...
              <label className="text-xl font-medium flex flex-col gap-5">
                ...
                <input
                  type="text"
                  defaultValue={form.supportGroupTopic}
              ...
              <label className="text-xl font-medium flex flex-col gap-5">
                ...
                <textarea
                  cols="30"
                  rows="12"
                  value={form.supportGroupSummary}
              ...
              <div className="grid md:grid-cols-3 grid-cols-1  items-center  gap-3">
                <div>
                  ...
                  >
                    <option selected={true}>{form.facilitator}</option>
                    {/* <option onClick={() => setSearchByUser("All")}>All</option> */}
                    {displayUserList()}
                  </select>
                </div>
                <div className="flex justify-center">
                  <label className="flex items-center text-xl gap-5 pt-7">
                    ...
                    <input
                      type="checkbox"
                      value={form.supportGroupSigned}
                      checked={
                        form.supportGroupSigned === true ? "checked" : ""
                      }
                      name="supportGroupSigned"
                      onChange={(e) => {
...
export const getServerSideProps = withPageAuthRequired({
  async getServerSideProps(ctx) {
    ...
    const [users, data] = await Promise.all([
      ...
      fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/support_groups/${ctx?.params?.id}`
      ).then((r) => r.json()),
    ]);
